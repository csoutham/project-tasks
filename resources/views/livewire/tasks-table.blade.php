	...
	<div class="flex flex-row pb-6">
		...
		<div class="w-full self-center hidden lg:block">
			<p class="mt-5 text-sm text-right lg:text-sm">
				Showing @if ($tasks->total() != 0)
					{{ $tasks->firstItem() }} to {{ $tasks->lastItem() }} of
				@endif
				{{ $tasks->total() }} results
			</p>
		</div>
                        ...
                        @foreach ($tasks as $task)
                            <tr class="hover:bg-gray-50">
                                <td class="pl-4 column-body">
                                    <a href="{{ action('\App\Http\Controllers\TasksController@edit', $task) }}" class="text-blue-600 font-semibold">
                                        {{ $task->title }}
                                    </a>
                                </td>
                                ...
                                <td class="column-body text-right pr-4">
                                    <form action="{{ action('\App\Http\Controllers\TasksController@destroy', $task) }}"
                                          method="post">
                                        @csrf
                                        @method('DELETE')
